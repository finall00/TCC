 ...
 * @author smili08
 */
@WebServlet(name = "RemoverItem", urlPatterns = {"/RemoverItem"})
public class RemoverItem extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int codigoItem = Integer.parseInt(request.getParameter("codigoP"));

            ArrayList<ItensVenda> carrinho = (ArrayList<ItensVenda>) sessao.getAttribute("itensProduto");
            if (carrinho != null) {
                for (ItensVenda car : carrinho) {
                    if (car.getProduto().getCodigoProduto() == codigoItem) {                        
                        carrinho.remove(carrinho.indexOf(car));
                        break;
                    }                   
                }              
            }
